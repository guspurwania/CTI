...
class Partnerlink extends CI_Controller {
    ...
    public function index()
    {
        ...
        {
    	   $data['users'] = Users::with('user')->where('group_id', 3)->get();
        }
        elseif($data['session']['group_id'] == 2)
        {
            $data['users'] = Users::with('user')->where('group_id', 3)->where('user_id', $data['session']['user_id'])->get();
        }
    	$data['url'] = $this->uri->segment(1);
    	$this->twig->display('partnerlink/index', $data);
    }
    ...
    public function create()
    {
    	...
    	$data['session'] = $this->session->all_userdata();
    	$data['url'] = $this->uri->segment(1);
    	$this->twig->display('partnerlink/create', $data);
    }
    ...
    public function store()
    {
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($identity, $password, $email, $data['additional_data']))
        {
            $message = $this->load->view('email/register_partner.php', $data, true);
    ...
    public function show($id)
    {
        ...
        $data['students'] = Students::with(['program', 'user'])->where('user_id', $id)->get();

        $data['url'] = $this->uri->segment(1);
        $this->twig->display('partnerlink/show', $data);
    }
    ...
    public function edit($id)
    {
    	...
    	$data['user'] = Users::find($id);
    	$data['url'] = $this->uri->segment(1);
    	$this->twig->display('partnerlink/edit', $data);
    }
